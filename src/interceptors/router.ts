...
const navigateToInterceptor = {
  invoke({ url }: { url: string }) {
    ...
    console.log('🚀 ~ invoke ~ userPermissionKeys:', userPermissionKeys)
    // 页面所需权限
    const pageRoutePaths = getPermissionKeysByPath(path) // ['vip']
    console.log('🚀 ~ invoke ~ pageRoutePaths:', pageRoutePaths)
    // 两个权限取交集，然后交集再与用户权限取补集
    const LackPermissionKeys = pageRoutePaths.filter(item => !userPermissionKeys.includes(item))
    console.log('🚀 ~ invoke ~ LackPermissionKeys:', LackPermissionKeys)
